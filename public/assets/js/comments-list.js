    ...
    var dt_user = dt_user_table.DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          responsivePriority: 4,
          render: function (data, type, full, meta) {
            var $username = full['commentUserName'],
              $email = full['commentUserEmail'],
              $image = full['commentUserPic'];
            if ($username && $email && $image) {
              // For Avatar image
              var $output =
                '<img src="' + assetsPath + 'images/user/' + $image + '" alt="Avatar" class="rounded-circle">';
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var userId = full.comment_id;
            return (
              '<div class="d-inline-block text-nowrap">' +
              '<button class="btn btn-sm btn-icon delete-record" onclick="deleteComment(' +
              userId +
...
function deleteComment(userId) {
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        url: 'comments-delete/' + userId,
        type: 'POST',
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
          var parentTr = $('#comment-deleted-' + data.userId).closest('tr');
          parentTr.remove();
          Swal.fire({
            icon: 'success',
            title: 'Deleted!',
