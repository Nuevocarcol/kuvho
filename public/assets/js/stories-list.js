      ...
      columnDefs: [
        { visible: false, targets: [0] },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            return '<span class="fw-medium">' + $create_date + '</span>';
          }
        },
        {
          // Photo and Video
          ...
          render: function (data, type, full, meta) {
            var $output = '';
            var $mediaURL = full['url'];

            if ($mediaURL) {
              if ($mediaURL.endsWith('.mp4') || $mediaURL.endsWith('.webm') || $mediaURL.endsWith('.ogg')) {
                // For video content
                $output =
                  '<video controls style="max-width: 100% !important; max-height: 9rem !important; width: auto; height: auto;">' +
                  '<source src="' +
                  assetsPath +
                  'images/posts/' +
                  $mediaURL +
                ...
                $output =
                  ...
                  'images/posts/' +
                  $mediaURL +
                  '" alt="Avatar" class="rounded-circle" style="max-width: 50px !important; height: 50px !important;">';
              }
            }
            return $output;
          }
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var userId = full.story_id;
            return (
              '<div class="d-inline-block text-nowrap">' +
              '<button class="btn btn-sm btn-icon delete-record" onclick="deleteStories(' +
              userId +
...
function deleteStories(userId) {
  Swal.fire({
    title: 'Are you sure?',
    text: "You won't be able to revert Story!",
    icon: 'warning',
    ...
    if (result.value) {
      $.ajax({
        url: 'stories-delete/' + userId,
        type: 'POST',
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
          var parentTr = $('#stories-deleted-' + data.userId).closest('tr');
          parentTr.remove();
          Swal.fire({
            icon: 'success',
            title: 'Deleted!',
